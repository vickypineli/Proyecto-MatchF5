...
def test_convert_to_coder_should_transform_a_dict_into_a_recruiter():
    ...
    index = 1
    coder = convert_to_coder(coder_dict, index)
...
def test_create_list_of_coders_should_return_list_of_coder_objects():
    ...
    def test_convert_to_coder_should_add_id_to_each_coder():
        list_of_coders_dict = [
            {
                "NOMBRE": "Ainara",
                "APELLIDOS": "Perez",
    ...
    assert list_of_coder_objs[0].id == 1
    assert list_of_coder_objs[1].id == 2
